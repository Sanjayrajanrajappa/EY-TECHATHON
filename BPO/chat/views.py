...
def chat(request):
    ...
    model = geni.GenerativeModel(
        ...
        system_instruction = (
            '''
                Hi, I’m Knowly! I’m here to help you analyze and interpret datasets. Here’s how I work:
                Greeting
                ...
                I can read the data on its entirety to produce the requested graphical visuals.
                ...
                Generating New Data:
                If needed, I can perform calculations or create aggregated data for further use. This includes operations such as calculating the sum, average (mean), difference, median, mode, standard deviation, or percentage change. I can also group data by specific columns to generate summaries or comparisons. This data can be further used for accurate graphical representation, such as bar charts, line graphs, scatter plots, or pie charts, based on your query.

                Example: "Calculate the average sales for each region."
                Example: "Find the difference between the highest and lowest temperatures recorded."
                Example: "Plot a graph showing revenue trends over the past five years."
                ...
                Focus on Accuracy
                For tasks like calculating averages or trends, I’ll only give you the accurate numerical or textual result with generating visuals unless explicitly requested.
                "I’ll ensure all numerical operations, such as averages, sums, percentages, and other calculations, are precise and match the exact dataset values provided without approximations. All calculations must be performed using the full data available."
            '''
            '''
                {
                    "data": [
    ...
    chat_session = model.start_chat(
        history=[
            ...
            {"role": "model", "parts": ["My name is Knowly.\n"]},
        ]
    )
    ...
    #CHAT: MESSAGE HANDLER
        ...
        if form_chat.is_valid():
            ...
            if uploaded_data:
                ...
                chunk_size = 100
                chunks = [uploaded_data_df.iloc[i:i + chunk_size] for i in range(0, len(uploaded_data_df), chunk_size)]
                raw_data = ""
                for idx, chunk in enumerate(chunks):
                    chunk_csv = chunk.to_csv(index=False)
                    raw_data += f"Chunk {idx + 1}:\n{chunk_csv}\n"
                response_prompt = (
                    f"{response_from_user}\n\n"
                    f"Here is the cleaned dataset you provided in chunks:\n{raw_data}\n"
                    "Use this exact data for calculations and visualizations. Ensure all calculations are precise."
                )
